...
import { listTopupIntents } from "@/lib/billing/stripe";
...
export async function GET(request: Request) {
  ...
  const intents = listTopupIntents(user.userId);
  return NextResponse.json({ topupIntents: intents, count: intents.length });
}
